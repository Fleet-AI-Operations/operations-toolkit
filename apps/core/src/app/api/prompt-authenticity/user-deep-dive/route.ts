...
// ============================================================================
// DEDUPLICATION
// For each task_key in metadata, keep only the DataRecord with the latest
// createdAt. Records missing a task_key are kept as-is.
// ============================================================================
function deduplicateByTaskKey<T extends { createdAt: Date; metadata: any }>(records: T[]): T[] {
  ...
  // Re-sort combined result chronologically
...
export async function GET(request: NextRequest) {
  ...
  // Deduplicate: for each task_key keep only the latest DataRecord.
  // Records without a task_key in metadata are kept as-is.
  ...
  // ── 2. Fetch analysis records for this user from PromptAuthenticityRecord ─
  ...
  return NextResponse.json({
    ...
    summary: {
      ...
      analyzed,
      aiGeneratedCount,
      aiGeneratedPct: pct(aiGeneratedCount),
      templatedCount,
      templatedPct: pct(templatedCount),
      nonNativeCount,
      nonNativePct: pct(nonNativeCount),
