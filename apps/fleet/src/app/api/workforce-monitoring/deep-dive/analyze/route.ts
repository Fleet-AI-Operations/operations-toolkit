 ...
 *
 * Syncs a worker's tasks into PromptAuthenticityRecord then runs AI analysis.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (allRecords.length === 0) {
      return NextResponse.json({ error: 'No tasks found for this worker' }, { status: 400 });
    }
      ...
      for (const chunk of chunks) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (r.status === 'fulfilled') {
            ...
            failed++;
            console.error('[workforce-monitoring/deep-dive/analyze] Record failed:', chunk[i].id, r.reason);
            await prisma.promptAuthenticityRecord.update({
              where: { id: chunk[i].id },
              data: {
              ...
              console.error('[workforce-monitoring/deep-dive/analyze] Failed to reset record to FAILED status:', chunk[i].id, resetErr);
            });
          }
        }
      }
    ...
    // Cross-prompt template analysis
      ...
      try {
        ...
        updateResults.forEach((r, i) => {
          if (r.status === 'rejected') {
            console.error('[workforce-monitoring/deep-dive/analyze] Template field update failed:', allCompleted[i].id, r.reason);
            templateAnalysisFailed = true;
          }
        });
      } catch (err) {
        console.error('[workforce-monitoring/deep-dive/analyze] Cross-prompt template analysis failed:', err);
        templateAnalysisFailed = true;
      }
    }

    const templateNote = templateAnalysisFailed ? ' Template analysis failed — template badges may be incomplete.' : '';
    ...
    return NextResponse.json({ synced: syncResult.count, analyzed, failed, total: dataRecords.length, templateAnalysisFailed, message });
  } catch (err: any) {
    console.error('[workforce-monitoring/deep-dive/analyze] POST failed:', err);
    if (email) {
      ...
      await prisma.promptAuthenticityRecord.updateMany({
        ...
        console.error('[workforce-monitoring/deep-dive/analyze] Failed to reset stuck ANALYZING records:', resetErr);
      });
    }
